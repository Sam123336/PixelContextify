...
| `trace_flow`      | User journeys as styled flow diagrams: `from`+`to` traces the path (navigation → screens → API calls, with side-branches dashed and a numbered step list with file paths); `from` alone renders the forward journey tree. A whole checkout flow ≈ 200–500 tokens instead of reading dozens of files |
| `explain_visually`| Multi-diagram Mermaid dossier for any node: how users reach it, what it's composed of, where its data comes from (API → hook → state → UI), a state-placement decision tree with this project's actual branch highlighted, and project-specific recommendations — architecture explanations for backend developers, generated from *your* graph, not generic tutorials |
| `what_if`         | Digital twin: simulate a change before making it — `remove` (what breaks, what's transitively at risk, which routes stay safe), `split` (call sites + natural boundaries), `lazy_load` (exclusive vs shared subtree, loading boundaries, worth-it verdict) |
